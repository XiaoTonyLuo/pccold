...
class Danmu(object):
    ...
    def sendMsg(self, msg):
        ...
        msg_head = int.to_bytes(data_length, 4, 'little')+int.to_bytes(
            data_length, 4, 'little')+int.to_bytes(self.codeLocalToServer, 4, 'little')
    ...
    def join(self):
        msg = 'type@=joingroup/rid@=' + \
            str(self.rid)+'/gid@='+str(self.gid)+'/\x00'
        self.sendMsg(msg)
        data = self.sock.recv(1024)
    ...
    def keeplive(self):
        while True:
            ...
            now_time = time.strftime(
                '_%m_%d_%H_%M', time.localtime(time.time()))
    ...
    def dataProcess(self, data):
        ...
        if d:
            if d.group(1) == b'chatmsg':
                ...
                try:
                    print(danmu.group(1).decode() +
                          ':::'+danmu.group(2).decode())
