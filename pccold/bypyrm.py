...
from pccold.tools import sendEmail
import psutil
import subprocess
import logging
...
def psCheck(name):
    result = [p.info for p in psutil.process_iter(
        attrs=['pid', 'name']) if name in p.info['name']]
    return result
...
def initBypyRmEmail(body):
    ...
    try:
        sendEmail(subj, conf.my_email, body, conf.mail_sender,
                  conf.mail_passwd, conf.mail_host, conf.mail_port)
